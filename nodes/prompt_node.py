...
class SnapshotPromptServer:
    ...
    def _load_programs(self):
        self._ensure_dirs()
        if os.path.exists(self.programs_json):
            try:
                with open(self.programs_json, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                if isinstance(data, dict):
                    # Migrate old 'applications' key to 'programs' in each category
    ...
    def _save_programs(self, data):
        self._ensure_dirs()
        try:
            with open(self.programs_json, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except:
            pass
        ...
        def do_POST(self):
            if self.path == '/select_prompt':
                ...
                if self.server_instance:
                    ...
                    app_image = data.get('image', None)
                    app_id = None
                    if app_name:
                        app_id = self.server_instance._generate_id()
                        preview = ''
                        if app_image:
                            preview = self.server_instance._save_image(app_image)
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({'status': 'ok', 'id': app_id, 'preview': preview if app_id else ''}).encode('utf-8'))
                else:
                    self.send_error(500, "Server error")

            elif self.path == '/update_program':
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length)
                data = json.loads(post_data)
                if self.server_instance:
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({'status': 'ok', 'preview': new_preview}).encode('utf-8'))
                else:
                    self.send_error(500, "Server error")

            elif self.path == '/update_program_category':
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length)
                data = json.loads(post_data)
                if self.server_instance:
                    ...
                    if old_name and new_name and old_name in self.server_instance.programs_data:
                        cat_data = self.server_instance.programs_data.pop(old_name)
                        if not isinstance(cat_data, dict):
                            cat_data = {'programs': cat_data}
                        if image_data:
                            cat_data['bg_image'] = self.server_instance._save_image(image_data)
                        elif image_data == '':
                            cat_data['bg_image'] = ''
                        ...
                        self.server_instance.programs_data[new_name] = cat_data
                        self.server_instance._save_programs(self.server_instance.programs_data)
                    self.send_response(200)
                    self.send_header('Content-type', 'application/json')
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({'status': 'ok', 'bg_image': self.server_instance.programs_data.get(new_name, {}).get('bg_image', '')}).encode('utf-8'))
                else:
                    self.send_error(500, "Server error")

            elif self.path == '/update_program_display_mode':
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length)
                data = json.loads(post_data)
                if self.server_instance:
                    ...
                    display_mode = data.get('display_mode', 'horizontal')
                    size_mode = data.get('size_mode', 'normal')
                    if cat_name in self.server_instance.programs_data:
                        cat_data = self.server_instance.programs_data[cat_name]
                        if not isinstance(cat_data, dict):
                            cat_data = {'programs': cat_data}
                            self.server_instance.programs_data[cat_name] = cat_data
                        cat_data['display_mode'] = display_mode
                        cat_data['size_mode'] = size_mode
                        self.server_instance._save_programs(self.server_instance.programs_data)
                    self.send_response(200)
                ...
                if self.server_instance:
                    ...
                    for cat_name, cat_data in self.server_instance.programs_data.items():
                        if isinstance(cat_data, dict):
                            cat_data['applications'] = [a for a in cat_data.get('programs', []) if a.get('id') != app_id]
                    self.server_instance._save_programs(self.server_instance.programs_data)
                    self.send_response(200)
                    self.send_header('Content-type', 'application/json')
                    self.send_header("Access-Control-Allow-Origin", "*")
            ...
            elif self.path == '/reorder_programs':
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length)
                data = json.loads(post_data)
                if self.server_instance:
...
class SnapshotPromptNode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "program_cache": ("BOOLEAN", {"default": False}),
                "program": ("STRING", {"default": "", "multiline": True}),
            },
            "optional": {
    ...
    def snapshot_prompt(self, prompt_cache, prompt, prompt_parsing, lora_cache, lora_path_mode, lora_regex, lora, prefab_cache, prefab, program_cache, program, unique_id,
        ...
        last_selected_programs = []
        if program and program.strip():
            try:
                last_selected_programs = json.loads(program.strip())
                if not isinstance(last_selected_programs, list):
                    last_selected_programs = []
            except Exception:
                last_selected_programs = []
        print(f"[SnapshotPrompt] Loading program widget: len={len(program or '')}, parsed_applications={last_selected_programs}")
        ...
        if program_cache:
            ...
            print(f"[SnapshotPrompt] Saving program cache: {program_cache_value[:300]}")
            PromptServer.instance.send_sync("kolid-comfy-widget-set", {
                "node_id": unique_id,
                "widget_name": "program",
                "type": "STRING",
                "value": program_cache_value
            })
