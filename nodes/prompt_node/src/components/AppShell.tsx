...
export function AppShell() {
  ...
  const handleDebug = useCallback(() => {
    ...
    try {
      ...
      // Builtin: random sample (no toggle needed — always available)
      const prefabBuiltin = shuffle([...prefabDataMap.entries()]).slice(0, randInt(2, 5)).map(([guid, pfData]) => ({
        guid, name: pfData.name || '', tag_groups: pfData.tag_groups || [], loras: pfData.loras || [], custom_prompts: pfData.custom_prompts || '', preview: pfData.preview || '', active: true,
      }));
  ...
  const isLoraSelected = useCallback((item: LoraItemData) => {
    if (tempCtx.mode === 'lora' || tempCtx.mode === 'loraCtx' || tempCtx.mode === 'loraBuiltin') {
  ...
  const applyLoadedData = useCallback((mode: 'merge' | 'replace') => {
    ...
    }).filter(g => g.source !== 'program'); // discard program-sourced — they're regenerated by useProgram
    ...
    try {
      ...
      for (const saved of loraArr) {
        ...
        if (loraSource === 'program') continue; // discard program-sourced — regenerated by useProgram
        if (item) {
    ...
    let restoreTree: ((node: any, visited: Set<string>) => SelectedPrefabItem | null) | null = null;
    try {
      const prefabArr = JSON.parse(loaded.prefab || '[]');
      restoreTree = function restoreTree(
        node: { guid: string; active?: boolean; tag_groups?: SelectedPrefabTagState[]; tags?: SelectedPrefabTagState[]; loras?: SelectedPrefabLoraState[]; children?: any[] },
        visited: Set<string>,
      ): SelectedPrefabItem | null {
        if (visited.has(node.guid)) return null;
        ...
        const children: SelectedPrefabItem[] = (prefab.selected_prefabs || [])
          ...
          .filter(Boolean) as SelectedPrefabItem[];
        const nodeSource = (node as any).source || 'normal';
        if (nodeSource === 'program') return null; // discard program-sourced — regenerated by useProgram
        return { guid: node.guid, active: node.active !== false, tag_groups, loras, children, source: nodeSource };
      }
    ...
    // program-sourced items are discarded (they're regenerated by useProgram)
    try {
      const filterTagsArr = typeof loaded.filter_tag_groups === 'string' ? JSON.parse(loaded.filter_tag_groups) : (loaded.filter_tag_groups || []);
      const existingTagKeys = new Set(newTags.map(g => tagsToDisplayString(g)));
      for (const ft of filterTagsArr) {
        const ftSource = ft.source || 'normal';
    ...
    try {
      const filterLorasArr = typeof loaded.filter_loras === 'string' ? JSON.parse(loaded.filter_loras) : (loaded.filter_loras || []);
      const available = new Map<string, LoraItemData>();
      for (const items of Object.values(loraData)) { for (const item of items) { available.set(item.file_path, item); } }
      for (const saved of filterLorasArr) {
        const lookupKey = saved.file_path || saved.file_name;
        if (newLoraSelections[lookupKey]) continue;
        const flSource = saved.source || 'normal';
        if (flSource === 'program') continue; // discard program-sourced — regenerated by useProgram
        const item = available.get(lookupKey);
        if (item) {
          newLoras.push({ ...item, source: flSource as any });
        } else {
          const derivedName = lookupKey.includes('/') ? lookupKey.split('/').pop()! : lookupKey;
    ...
    try {
      const filterPrefabsArr = typeof loaded.filter_prefabs === 'string' ? JSON.parse(loaded.filter_prefabs) : (loaded.filter_prefabs || []);
      const existingPrefabGuids = new Set(newPrefabs.map(p => p.guid));
      for (const fp of filterPrefabsArr) {
        if (existingPrefabGuids.has(fp.guid)) continue;
        if ((fp.source || 'normal') === 'program') continue; // discard program-sourced — regenerated by useProgram
        const item = restoreTree ? restoreTree(fp, new Set()) : null;
        if (item) { newPrefabs.push(item); existingPrefabGuids.add(fp.guid); }
      }
    } catch {}
      ...
      <div className="main-wrapper">
        <div className="scroll-container">
          <div className="container">
            ...
            {/* ========== Lora Section ========== */}
            {(!tempCtx.mode || tempCtx.mode === 'lora' || tempCtx.mode === 'loraCtx' || tempCtx.mode === 'loraBuiltin') && Object.keys(loraData).length > 0 ? (() => {
            ...
            {(!tempCtx.mode || tempCtx.mode === 'prefab' || tempCtx.mode === 'prefabCtx' || tempCtx.mode === 'prefabBuiltin') ? (<div className="categories-container prefab-section">
                      ...
                      <div className={`category-content${anim ? ' animating' : ''} ${displayMode==='box'?'box-mode':''} ${isMiniMode?'mini-mode':''}`}>
                        ...
                        {filteredPrefabs.map((pf, i) => (
                          <PrefabItem key={`prefab_${lib}_${i}`}
                            ...
                            isSelected={tempCtx.mode === 'prefab' || tempCtx.mode === 'prefabCtx' || tempCtx.mode === 'prefabBuiltin' ? tempCtx.isIdSelected(pf.guid || '') : selectedPrefabs.some(p => p.guid === pf.guid)}
                            onToggle={() => { if (tempCtx.mode === 'prefabCtx' || tempCtx.mode === 'prefabBuiltin') { tempCtx.toggleId(pf.guid || ''); } else { togglePrefab(pf.guid || `${lib}_${i}`); } }}
